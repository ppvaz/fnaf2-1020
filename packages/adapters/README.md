...
Commands: use the root `test:contracts` and `device:bench` lanes. Artifacts: profile-resolved capability and calibration metadata.

The fixture adapters are hermetic and are used by the conformance lane. A legal HID/ADB send is not evidence that the game accepted
