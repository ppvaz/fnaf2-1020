...
| Open-loop stock-device runner | [`tools/device/trial-minus7.sh`](../../tools/device/trial-minus7.sh), on-device monotonic HID multitouch plan, epoch latch, preflight/focus/watchdog checks | A model-gated Night 6 route exists. It is not a demonstrated 10/20 or full-night clear. |
