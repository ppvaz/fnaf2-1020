...
| **Today: the model gate passed on its seed block** | Not run telemetry. Recorded here only because it changes the target: Night 6 is refused at **449/1200 = 37.4%** against a 40% contract (`plans/13:175-189`), so the next runs are Nights 1-5 (99.1, 66.5, 77.1, 72.3, 62.5%). If any telemetry proposal ever samples, **sample scattered** — `tools/cyclesearch.mjs` uses `SEED = (i) => (i * 2246822519) >>> 0`; sequential blocks are what produced the false pass | n/a |
