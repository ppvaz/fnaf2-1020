...
build_session() {                               # RUN LIFECYCLE
  ...
  (
    ...
    fnaf_session_begin "$run" "tools/device/collect-cue-audio.sh" || exit 9
...
grep -q 'source "$HERE/session.sh"' "$HERE/collect-cue-audio.sh"
check "collect-cue-audio.sh sources session.sh" $?
grep -q 'fnaf_session_begin "$LABEL"' "$HERE/collect-cue-audio.sh"
