...
// The atomic helper observation must not cost the mask any latency: it is
// launched in the background alongside screencap and reaped only after the
// mask press has gone out. A serial `cue_frame` before the mask would add ~53 ms
// to the mask-off seam the census already showed is where reads are lost.
const gridLaunch = readBody.indexOf('cue_frame > "$capture_frame" &');
...
check(/\$\{classification%% \*\}\.frame/.test(readBody),
  'every read must retain its atomic helper observation, empty class included');
check(!/cl_snap=\$\(cue_snapshot\)/.test(readBody),
  'telemetry must reuse the parallel FRAME, not delay the response with another GET');
