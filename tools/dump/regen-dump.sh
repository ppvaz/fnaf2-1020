...
# docs/android/SOURCE-DUMP-GUIDE.md); the dumper writes the file named by CTFAK_EVENT_DUMP.
set -euo pipefail
...
[ -f "$CLI" ] || { echo "no CTFAK build at $CLI (see docs/android/SOURCE-DUMP-GUIDE.md)" >&2; exit 1; }
