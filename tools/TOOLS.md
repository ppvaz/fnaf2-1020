...
| `tools/hidpilottest.mjs [runs] [--night=6]` | report | Exact-simulator report for the selected phase-safe lit-left-opening HID policy; it passes 10000 ordinary and 3000 pinned-worst Night 6 seeds. `--tick-aligned-mask` restores the unsafe clock-aligned bound, `--always-threat` is a false-positive negative control, `--cam5` selects the every-cycle comparison route, and `--sparse-cam5` waits until BB can reach CAM 05 before checking each movement boundary. `--cam5-light-ms=N` is a diagnostic power-bound knob; it does not validate that capture duration on-device. `--no-bb`/legacy `--no-cam5` deliberately remove BB handling. `--hypothetical-unlit` is rejected, not a usable controller. |
