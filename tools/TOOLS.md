...
| `tools/device/trial.sh [name] [cycles]` | **device action** | The sole device route: it emits, gates, and executes the selected policy through `hid-multi` with a device-epoch latch. `DEVICE_POLICY=minus-toys` requires an explicit `MINUS_TOYS_VARIANT=standard\|minimal`; `minimal` is Night 1 only. `CUE_AUDIO=1` (with `CUE_HELPER=1`) retains PCM. Capture duration comes from the emitted plan, including a declared observation tail only after the terminal phase is reached. Automatic video grading is **off by default** (`GRADE_RUN=0`); use `GRADE_RUN=1` only as an explicit, resource-aware diagnostic or run the grader separately. Capture duration is negotiated from `screenrecord --help`: full nights require advertised unlimited `--time-limit 0`, and legacy 180-second-only devices fail before launch rather than emit incomplete evidence. **Run:** `BB_LEFT_MODEL=captures/screencheck/bb-left/models/runtime-gh.scm tools/device/trial.sh NAME 90`. |
...
| `tools/device/policy-grammar.mjs` | module | Finite structural grammar for policy-v1: named sourced setup target, ordered five-phase shape, engine-shaped action constraints, and known-family fingerprints. |
...
| `tools/device/test-trial-reactive.sh` | check | Phone-free static regression for `trial.sh`'s observe-only reactive branch, watchlist loading, READ polling, manifest fields, and immediate refusal of unproven action mode. |
...
| `tools/device/cam11lit.py` | device action | Classifies whether the CAM 11 map button is lit on a monitor screencap (PNG on stdin or path): the arm verifier for the Minus Toys `--minimal` opening, since the button highlight is driven from `viewing` by g46-57 and lit ⇔ the raise restored CAM 11. Green-mean bands 170/140 measured on the 2026-08-29 r2/r3 recordings (lit 228.0-229.7, unlit 110.2-111.8, office 34 / menu 16 as never-lit controls); between them it answers `unknown` instead of voting. Prints one self-describing `verdict=` line; exits 0. |
| `tools/device/test-cam11lit.sh` | check | Runs cam11lit.py over committed crop fixtures (lit/unlit from r2/r3, office/menu as never-lit controls) composited into full frames at the instrument's normalized fractions, plus a portrait-rotated case pinning the rotation convention. No device. |
