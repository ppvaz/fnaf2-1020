...
| `tools/pilottest.mjs [n]` | report/check | Replays the phone pilot schedule in the simulator. Options include `--vent`, `--sync`, `--evict`, `--late-flash`, `--cycles=N`, `--night=N`, and `--worst`; `--assert` guards the narrow BB→Foxy claim rather than full survival. `--night=6` is the night the device actually plays. `--device-actuator` prices the run through `tools/device/actuator.mjs` (per-press launch lateness plus the mask-seam monitor drop; `--press-late-ms=MIN,MAX` overrides the measured band). |
...
| `tools/device/trial-minus7.sh [name] [cycles]` | **device action** | Guarded, recorded timed loop with experimental clean-frame controls `BB_LEFT_CAPTURE_*`, `BB_LEFT_MODEL`, and `GF_OFFICE_MODEL`. `DEVICE_EPOCH_LATCH=1` timestamps two consecutive clock+flashlight HUD frames on-device and saves the clear/positive bracket. `HID_LEFT_SURVIVAL=1` is limited to four pre-read epoch/790 ms sweep-probe cycles because that phone-accepted sweep makes the sparse policy fail every exact Night 7 simulation; it cannot make a BB decision. `CUE_AUDIO=1` (with `CUE_HELPER=1`) keeps the night's PCM through the helper's log capture, which is the only way a bang scan has any input at all -- `screenrecord` is video-only and the cue trace's rms/peak cannot carry a transient. Other controls include `NIGHT`, `DEBUG_OVERLAYS`, `GRADE_RUN`, `PRESS_MODE`, and watchdog intervals.  **Canonical run:** `DEBUG_OVERLAYS=0 PRESS_MODE=hid BB_LEFT_CAPTURE_EVERY=4 BB_LEFT_MODEL=captures/screencheck/bb-left/models/candidate-runs-gh.scm tools/device/trial-minus7.sh NAME 60`. **Night 6 Left-Read run:** `DEBUG_OVERLAYS=0 PRESS_MODE=hid-multi NIGHT6_LEFT=1 DEVICE_EPOCH_LATCH=1 BB_LEFT_MODEL=captures/screencheck/bb-left/models/runtime-gh.scm tools/device/trial-minus7.sh NAME 90` -- use `runtime-gh.scm` here, not `candidate-runs-gh.scm`: that model's frames come from `capture_lit_at`'s fixed 350 ms delay and do not cover this loop's latch window. Running with no `BB_LEFT_MODEL`/`BB_CAM05_MODEL` is a known-dead configuration (0/3000 via BB->Foxy) and the script now warns; CAM 05 is not the Night 6 checkpoint. |
...
| `tools/device/test-runner-plan.mjs` | check | Gates that the runner *interprets* the plan rather than carrying a second copy: no schedule literals in the NIGHT6_LEFT driver, the host pushes what `recipe.mjs --device-plan` emits, the branch windows line up with the plan's instructions, and the plan itself replays through the engine 300/300. No device. |
| `tools/device/human-gate.mjs plan.txt` | check | The model gate: replays the plan through the exact engine over 100 seeds with every press shifted by ±60 ms of human slack (the measured plans/04 bracket floor, until the trace census supersedes it with correlated bands) and refuses below the 40% replay contract. `trial-minus7.sh` runs it before its FIRST adb command — nothing reaches the phone unless locally proven — and refuses inline-schedule modes outright as unpriceable. Absolute, no override (2026-08-25). |
| `tools/device/test-human-gate.mjs` | check | Mock regression for the model gate plus the decision record: plan-text round-trip, bounded deterministic error injection, exact verdict thresholding, the shipped Night 6 plan refused (23/100), and both runner invocations (inline and plan mode) refusing pre-flight with no device attached. |
