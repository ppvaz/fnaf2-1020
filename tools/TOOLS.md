...
| `tools/hidpilottest.mjs [runs] [--night=6]` | report | Exact-simulator report for the selected phase-safe lit-left-opening HID policy; it passes 10000 ordinary and 3000 pinned-worst Night 6 seeds. `--tick-aligned-mask` restores the unsafe clock-aligned bound, `--always-threat` is a false-positive negative control, `--cam5` selects the comparison route, and `--no-bb`/legacy `--no-cam5` deliberately remove BB handling. `--hypothetical-unlit` is rejected, not a usable controller. |
