...
| `tools/device/trial-minus7.sh [name] [cycles]` | **device action** | Guarded, recorded timed loop with experimental clean-frame controls `BB_LEFT_CAPTURE_*`, `BB_LEFT_MODEL`, and `GF_OFFICE_MODEL`. A model result other than confident `empty` masks and aborts before the hall. Other controls include `NIGHT`, `DEBUG_OVERLAYS`, `GRADE_RUN`, `PRESS_MODE`, and watchdog intervals. It refuses capture overwrite and stops on lost focus/night state. |
