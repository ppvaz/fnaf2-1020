...
| `tools/minus7/paramsearch.mjs --nights=N,N --shape=correlated [--runs --admit --beam --rounds]` | search/report | Plan 16's constrained policy search: dominance-pruned beam over `hidpilottest.mjs` `SEARCH_KNOBS` (all default-inert -- the 803feb3 plan is byte-identical with every knob 0), evaluated `recipe.build -> devicePlan -> modelGate`. Pareto frontier on the per-night survival + seed-CVaR vector; `--admit` re-scores the frontier at 1200 seeds. `--shape` selects the human-gate slack model. Exports `baselineLadder`, `evalParams`, `searchParams`, `FLOORS`. |
...
| `tools/minus7/test-search.mjs` | check | Plan 16 pkg 1/3 gates: `Sim.snapshot()/restore()` bit-identity, every semantic action runs, and `paramsearch` reproduces the 803feb3 ladder on a zero perturbation. Runs in `tools/test.mjs --engine`. |
...
| `tools/device/sweepcheck.py VIDEO [--signature F]` / `--recalibrate ALT.mp4` | check | Did the sweep **flash** each camera, not merely select it? Per-camera classifier over the feed CENTRE: CAM 07 (Main Hall, black without light) splits on `vedge` (edge density), CAM 10/04 on `brightfrac`. `--recalibrate` learns the thresholds from an `ALT_LIGHT=1` run (even sweeps lit / odd dark under one night's content). Bundled signature `sweepcheck-signature.json` is from `c33-alt.mp4`; on that control 24/25 sweep verdicts correct and the `NO_LIGHT` control is 0/25 false positive. The old brightness measure was coin-flip on the c33 geometry and 5/73 on a *cleared* night. |
