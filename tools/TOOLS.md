...
| `tools/device/human-gate.mjs plan.txt` | check | The human floor's plan auditor: refuses any device plan scheduling press gaps under `HUMAN_GAP_FLOOR_MS` (350 ms, `[INFERRED]` from the duel gate until the trainer trace census supersedes it). Absolute, no override (2026-08-25); `trial-minus7.sh` pre-flights every plan through it, and its live sibling inside `press_at`/`hold_at`/`pulsed_sweep_at` covers schedules that never pass through a plan file. |
| `tools/device/test-human-gate.mjs` | check | Mock regression for the plan auditor plus the decision record: pins the floor's two copies equal, verifies the runner wiring, and asserts the shipped 120 ms Night 6 plan is refused. No device. |
