...
- Chowdren is a viable CCN-to-C++ research route, but it uses a reimplemented
  ...
  `write_foreach` and group-pointer resolution; event C++ emission now completes
  for the 29 real frames (the four truncated debug frames are skipped). The
  unsupported-ACE inventory is the next desktop-build repair list.
  See
...
| faithful Chowdren recompile | Run the redacted parser → C++ → desktop-boot contract in [`IN-ENGINE-PILOT-RECOMPILE.md`](../docs/in-engine/IN-ENGINE-PILOT-RECOMPILE.md#recompile-probe-contract-and-evidence-boundary). | First `GameData` parse, then generated C++, desktop boot, and finally an in-process state trace — each is a separate milestone. | Stop at the first failing phase and preserve its derived log; do not call a parser success, generated source, or rebuilt desktop run a stock-APK result. **At 2026-08-28: parse ✓, assets ✓, C++ emission ✓ for 29 real frames, desktop link ✓, boots to the 02-title screen ✓.** Under Xvfb+llvmpipe with `ALSOFT_DRIVERS=null` and CWD at the asset dir, the binary boots frame 0 → frame 1 and runs the FNaF 2 title event logic (title text, `12:00 AM` clock, WARNING block, camera map, menu buttons), stable 45 s+. Sprites are placeholder boxes (image bank incomplete); no gameplay yet. Fidelity `rebuilt-runtime`. |
...
Cleared 2026-08-28: parse, assets, `write_objects`, numeric loops, group
pointers, extension-instance fallback, and source emission for the 29 real
frames. The generated code is externally held and includes documented inert
compatibility paths.
...
1. Obtain a display-capable external runtime probe or symbolize the dummy-driver
   ...
   driver, not the code.
2. **Frame-1 instance resolution fixed 2026-08-28.** The referenced objects
   genuinely aren't on frame 1 (Fusion Globals placed later, dead cross-frame
   refs) — Chowdren dereferenced `back_obj` unconditionally. Landed: absent
   single-object ACEs → type default / skipped no-op; `JumpToFrame` &c. no longer
   dropped (frame transitions emit); every object gets `create_alterables()`;
   `Media::play_id` guards `INVALID_ASSET_ID`. The binary now boots to the
   02-title screen. **Current boundary:** the image bank / `get_missing_image`
   path is incomplete so sprites draw as placeholder boxes, and the menu→night
   transition has not been reached. Next: image-bank decode so sprites render,
   then drive the menu to a night frame and compare to the sourced model before
   any pilot claim.
