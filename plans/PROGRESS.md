...
frames are skipped). Its unsupported-ACE inventory is the Phase-3 repair list;
the next boundary is desktop compilation, then boot. Generated code has not
been compiled or claimed faithful.
...
- **PAIRIP posture corrected** — `plans/17` §"PAIRIP-specific refresh". The VM is
  ...
  **Superseded 2026-08-28 (Pedro's call):** the runtime-attachment route is
  ...
  route is declined.
- **Recompile fallback (route 5) is now the active in-engine path; Phase 3 first
  ...
  overload. A Linux CMake probe configures and reaches generated event translation,
  then stops before link on unsupported generic extension methods and system
  actions without valid receivers. This establishes source emission only, not a
  ...
  emission" and §"Phase 3 — first arm64 compiler boundary".
