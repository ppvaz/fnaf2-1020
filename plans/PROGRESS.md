...
| [18 — modern tooling](18-modern-tooling.md) | 0 / 9 | **0%** | **Proposed 2026-08-28; Package 5 host/capture foundation landed, direct-HID follow-up not closed.** Nine additions, each tied to a documented failure and none adding a runtime dependency or a build step. The phone-free parser is tested; three current direct-HID traces had no app MotionEvent rows, so they cannot establish dispatch or frame landing. | Reproduce the earlier positive input-trace configuration and place each camera-select event on an actual frame landing; in parallel, Package 1 (`shellcheck` + the three footgun fixtures). |
