...
| P9 — compatibility removal/audit | Open | descriptive `tools/model/` pilots, no production test imports, TypeScript shape check plus checked JS (strict migration still open), `test:affected`, bounded progressive runner | Apply strict TypeScript to new/materially extracted long-lived modules or document an approved exception; resolve the known red engine gate, characterize the remaining legacy shell lane, and complete live qualification. |
